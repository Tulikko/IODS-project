...
summary(Boston)
```
Like we can see from the min/max/mean values, while all the data are numerical, the scale is very wide, ranging from sub-zero values to over seven hundred. Clustering values with such differences in scale can prove to be challenging, but lets test that out first. 
...
```
The correlation plot sure is pretty, and it seems like strongest correlation with our variable of interest, crime rate (crim), is with "tax" and "rad" variables. Let's see if this changes after standardization.
...
ggpairs(Boston, mapping = aes(col = cluster))

# Same with pairs function, not quite as pretty
pairs(Boston, col = cluster)
